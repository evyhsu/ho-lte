...
//yen
const char* g_ueRrcStateName2[LteUeRrc::NUM_STATES_igmp] = 
  {
    "CONNECTED_ON",
    "CONNECTED_OFF"
  };

std::string ToString (LteUeRrc::IgmpState s)
{
  return std::string (g_ueRrcStateName2[s]);
}

//yen
const char* g_ueRrcStateName3[LteUeRrc::NUM_STATES_receive] = 
  {
    "RECEIVE_OFF",
    "RECEIVE_ON"
  };

std::string ToString (LteUeRrc::ReceiveState s)
{
  return std::string (g_ueRrcStateName3[s]);
}
...
LteUeRrc::LteUeRrc ()
  : m_cphySapProvider (0),
    ...
    m_receivestate (RECEIVE_OFF),//yen
    m_imsi (0),
    m_rnti (0),
    m_cellId (0),
    m_multicastgroup (50),//yen
    m_useRlcSm (true),
...
/*--------------------yen add----------------------*/
void
LteUeRrc::SetUeAddress (Ipv4Address ueAddr)
{
...
void
LteUeRrc::SetRemoteAddress (Ipv4Address remoteAddr)
{
...
Ipv4Address
LteUeRrc::GetRemoteAddress (void)
{
...
/*--------------------yen add----------------------*/
void 
LteUeRrc::SetMulticastGroup (uint16_t multicastgroup)
{
...
uint16_t
LteUeRrc::GetMulticastGroup (void)
{
  NS_LOG_FUNCTION ("m_multicastgroup" << m_multicastgroup);
...
LteUeRrc::IgmpState
LteUeRrc::GetIgmpState (void)
{
  NS_LOG_FUNCTION (this);
  return m_igmpstate;
}

LteUeRrc::ReceiveState
LteUeRrc::GetReceiveState (void)
{
  NS_LOG_FUNCTION (this);
...
{
  NS_LOG_FUNCTION (this << packet << bid);
//yen
  NS_LOG_FUNCTION ("yen bid: " << bid);

  uint8_t drbid = Bid2Drbid (bid);
//yen
  NS_LOG_FUNCTION ("yen drbid: " << drbid);
...
/*yen add*/
void
LteUeRrc::DoSendigmp (uint8_t bid)
{
  NS_LOG_FUNCTION (this << "DoSendigmp");

  SwitchToReceiveState(RECEIVE_ON);

  Ptr<Packet> packet = Create<Packet>(); 

  Ipv4Address destination = GetRemoteAddress ();
  Ipv4Address source = GetUeAddress ();
  ...
  NS_LOG_LOGIC (this << " RNTI=" << m_rnti << " sending " << packet << "on DRBID " << (uint32_t) drbid << " (LCID" << params.lcid << ")" << " (" << packet->GetSize () << " bytes)");
  NS_LOG_FUNCTION ("identification" << m_identification);
  it->second->m_pdcp->GetLtePdcpSapProvider ()->TransmitPdcpSdu (params);  

}

void
LteUeRrc::SetBid (uint8_t bid)
{
...
uint8_t
LteUeRrc::GetBid () const
{
  NS_LOG_FUNCTION (this << m_bid);
  return m_bid;
}
...
LteUeRrc::DoReceivePdcpSdu (LtePdcpSapUser::ReceivePdcpSduParameters params)
{
  ...
  Ptr<Packet> packet = params.pdcpSdu;//yen
/*-------yen write-----------*/
  Ptr<Packet> pCopy = packet->Copy ();
  Ipv4Header ipv4Header;
  pCopy->RemoveHeader (ipv4Header);
  Ipv4Address remoteAddr =  ipv4Header.GetDestination ();
  ...
  NS_LOG_LOGIC ("Destination: " << remoteAddr <<" Identification: " <<Identification << " Protocol:" << Protocol << " SourceAddr :" << ueAddr);
/*-------yen end-----------*/
//yen add to make sure receive igmp If not send again
  ...
  switch (m_receivestate)
    ...
    case RECEIVE_ON:
          if (Protocol == 2)
                    {
            NS_LOG_FUNCTION ("this function is IGMPReply received by UE");
            SwitchToReceiveState(RECEIVE_OFF);
                    }
          else
                    {
            //YEN
            NS_LOG_FUNCTION ("DO not receive IGMPReply retransmit");
            uint8_t bid = GetBid ();
            DoSendigmp (bid);
                    }
...
//yen
        /*LteRrcSap::RrcIGMPRequest msg2;
        msg2.ueIdentity = m_imsi;
        m_rrcSapUser->SendRrcIGMPRequest (msg2);*/
      }
      break;
    ...
    case CONNECTED_HANDOVER:     
      {
        ...
        //YEN add this is when handover done , ue have to send an new igmp for router
        uint8_t bid = GetBid ();
        DoSendigmp (bid);
    ...
    case CONNECTED_NORMALLY:
      if (msg.haveMobilityControlInfo)
      ...
      else
        {
          ...
          NS_LOG_FUNCTION ("UE-RRC SendRrcConnectionReconfigurationCompleted");
        }
      break;
...
//yen
void
LteUeRrc::Stoppackettransmittion ()
{
  NS_LOG_FUNCTION (this);
  ...
  IgmpHeader igmpheader; 
  igmpheader.SetType (17); 
  igmpheader.EnableChecksum();
  igmpheader.SetGroupAddress(destination);  
  Ipv4Header ipHeader; 
  ipHeader.SetDestination(destination); 
  ipHeader.SetSource(source); 
...
LteUeRrc::ApplyRadioResourceConfigDedicated (LteRrcSap::RadioResourceConfigDedicated rrcd)
{
  ...
  //yen
          NS_LOG_INFO ("yen m_bid2DrbidMap:" << dtamIt->drbIdentity);
          //NS_LOG_INFO ("yen m_bid2DrbidMap END:" << m_bid2DrbidMap.end ());
          m_drbMap.insert (std::pair<uint8_t, Ptr<LteDataRadioBearerInfo> > (dtamIt->drbIdentity, drbInfo));
//yen
          std::map<uint8_t, Ptr<LteDataRadioBearerInfo> >::iterator yenit;
          for (yenit = m_drbMap.begin (); yenit != m_drbMap.end (); ++yenit)
          
                        {
                NS_LOG_INFO ("yen m_drbMap first: " << yenit->first << " second: " << yenit->second);  
                        }
          struct LteUeCmacSapProvider::LogicalChannelConfig lcConfig;
...
//reference LteUeRrc::DoSendData -> EpcUeNas::Send
{
...
/*if (ToString (oldState)=="IDLE_CONNECTING" && ToString (newState)=="CONNECTED_NORMALLY")
{
  NS_LOG_FUNCTION ("yen i am in");
  //DoSendigmp ();
}*/ //this can't work because bid isn't right
...
//yen add
void 
LteUeRrc::SwitchToIgmpState (IgmpState newState)
{
  NS_LOG_FUNCTION (this << newState);
...
//yen add
void 
LteUeRrc::SwitchToReceiveState (ReceiveState newState)
{
  NS_LOG_FUNCTION (this << newState);
  ReceiveState oldState = m_receivestate;
