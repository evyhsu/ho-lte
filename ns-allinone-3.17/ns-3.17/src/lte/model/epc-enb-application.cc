...
/*---------yen add-----------*/
EpcEnbApplication::EpsFlowId_t::EpsFlowId_t (const uint16_t a, const uint8_t b, const uint16_t c)
  : m_rnti (a),
    m_bid (b),
    m_multicastgroup (c)
{
...
EpcEnbApplication::DoPathSwitchRequest (EpcEnbS1SapProvider::PathSwitchRequestParameters params)
{
  ...
  NS_LOG_FUNCTION ("params.rnti" << params.rnti <<"params.mmeUeS1Id" << params.mmeUeS1Id <<"imsi" <<mmeUeS1Id);//yen
  // side effect: create entry if not exist
...
void 
EpcEnbApplication::DoIGMPReport (EpcEnbS1SapProvider::IGMPReportParameters params)
{
  NS_LOG_FUNCTION ("yen DoIGMPReport");
  uint16_t enbUeS1Id = params.rnti;  
  uint64_t mmeUeS1Id = params.mmeUeS1Id;
  uint64_t imsi = mmeUeS1Id;
  NS_LOG_FUNCTION ("params.rnti" << params.rnti <<"params.mmeUeS1Id" << params.mmeUeS1Id <<"imsi" <<mmeUeS1Id);//yen
  ...
  uint16_t gci = params.cellId;//target cellid
  std::list<EpcS1apSapMme::ErabSwitchedInDownlinkItem> erabToBeSwitchedInDownlinkList;
  for (std::list<EpcEnbS1SapProvider::BearerToBeSwitched>::iterator bit = params.bearersToBeSwitched.begin ();
       bit != params.bearersToBeSwitched.end ();
       ++bit)
...
void 
EpcEnbApplication::DoInitialContextSetupRequest (uint64_t mmeUeS1Id, uint16_t enbUeS1Id, std::list<EpcS1apSapEnb::ErabToBeSetupItem> erabToBeSetupList, uint16_t multigroup)
{
  NS_LOG_FUNCTION (this);
...
//YEN
      NS_LOG_FUNCTION ("YEN imsiIt->second" << rnti);
      uint16_t multicastgroup = 20;//yen
      struct EpcEnbS1SapUser::DataRadioBearerSetupRequestParameters params;
      params.rnti = rnti;
      params.multicastgroup = multicastgroup;//yen
      params.bearer = erabIt->erabLevelQosParameters;
      params.bearerId = erabIt->erabId;
      params.gtpTeid = erabIt->sgwTeid;
      NS_LOG_FUNCTION ("YEN multicastgroup" << params.multicastgroup << "params.rnti" << params.rnti);
      m_s1SapUser->DataRadioBearerSetupRequest (params);

      //EpsFlowId_t rbid (rnti, erabIt->erabId);
      EpsFlowId_t rbid (rnti, erabIt->erabId, multicastgroup);
      // side effect: create entries if not exist
      m_rbidTeidMap[rnti][erabIt->erabId] = params.gtpTeid;
      m_teidRbidMap[params.gtpTeid] = rbid;
//yen
        NS_LOG_FUNCTION ("yen_teid:" << params.gtpTeid << "rnti" <<rnti << "erabIt->erabId" << erabIt->erabId);
  for (std::map<uint32_t, EpsFlowId_t> ::iterator yenit = m_teidRbidMap.begin ();
       yenit != m_teidRbidMap.end ();
       ++yenit)
        {
        NS_LOG_FUNCTION ("yen m_teidRbidMap it->first" << yenit->first);
        }
  for (std::map<uint16_t, std::map<uint8_t, uint32_t> >::iterator it = m_rbidTeidMap.begin ();
       it != m_rbidTeidMap.end ();
       it++)
        {
        NS_LOG_FUNCTION ("yen m_rbidTeidMap it->first" << it->first);
        }
    }
}
...
void 
EpcEnbApplication::RecvFromLteSocket (Ptr<Socket> socket)//enb->sgw
{
  NS_LOG_FUNCTION (this);  
...
void 
EpcEnbApplication::RecvFromS1uSocket (Ptr<Socket> socket)//sgw->enb
{
  ...
  Ptr<Packet> packet = socket->Recv ();
//yen
  NS_LOG_FUNCTION ("yen RecvFromS1uSocket packet:" << packet);
  GtpuHeader gtpu;
...
//yen
  SendToLteSocket (packet, it->second.m_rnti, it->second.m_bid, it->second.m_multicastgroup);
}
...
void 
EpcEnbApplication::SendToLteSocket (Ptr<Packet> packet, uint16_t rnti, uint8_t bid, uint16_t multicastgroup)
{
  NS_LOG_FUNCTION (this << packet << rnti << (uint16_t) bid << packet->GetSize () << "multicastgroup" << multicastgroup);  
  EpsBearerTag tag (rnti, bid, multicastgroup);
  NS_LOG_FUNCTION ("tag.GetMulticastGroup" << tag.GetMulticastGroup () ) ;
  packet->AddPacketTag (tag);
  int sentBytes = m_lteSocket->Send (packet);
  NS_ASSERT (sentBytes > 0);
}
...
void 
EpcEnbApplication::SendToS1uSocket (Ptr<Packet> packet, uint32_t teid)//enb->sgw
{
  ...
  NS_LOG_FUNCTION ("yen SendToS1uSocket yes sendto"); 
}
