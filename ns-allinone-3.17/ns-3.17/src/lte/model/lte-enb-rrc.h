...
public:
  ...
  UeManager (Ptr<LteEnbRrc> rrc, uint16_t rnti, State s, uint16_t multicastgroup);
...
//yen
  void SetMulticastGroup (uint16_t multicastgroup);
  /** 
...
//yen
  void SendData (uint8_t bid, Ptr<Packet> p, uint16_t m_multicastgroup);
  ...
  void RecvRrcIGMPRequest (LteRrcSap::RrcIGMPRequest msg);
  ...
  void Stoppackettransmittion ();//yen
  /** 
   * 
...
//yen
  uint16_t GetMulticastGroup (void);
   ...
   */
  State GetState ();
  //void SwitchToState (State s);//yen change private to public
  //State m_state;
void SwitchToState ();//yen
private:
...
//yen
  Ptr<UeManager> GetUeManager (uint16_t rnti, uint16_t multicastgroup);
  /**
...
//yen
  /*void SetUEaddress (Ptr<Packet> packet);
  Ipv4Address  GetUEaddress ();*/
/*---------yen add-------------*/
  void SetUeAddress (Ipv4Address ueAddr);
  Ipv4Address GetUeAddress (void);

  void SetRemoteAddress (Ipv4Address remoteAddr);
  Ipv4Address GetRemoteAddress (void);

  void SendIGMPReport (uint16_t rnti, uint16_t cellId, Ptr<Packet> packet);
...
//yen
  ...
  void DoRecvRrcIGMPRequest (uint16_t rnti, LteRrcSap::RrcIGMPRequest msg);//yen
...
//YEN
  std::map <uint8_t, Ptr<LteDataRadioBearerInfo> > m_drbMap;  
//yen
  Ptr<EpcSgwPgwApplication> m_sgwpgw;
  struct X2uTeidInfo
  {
    uint16_t rnti;
    uint8_t drbid;
