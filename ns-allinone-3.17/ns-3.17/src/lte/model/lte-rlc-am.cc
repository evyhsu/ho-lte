...
{
  NS_LOG_FUNCTION (this << m_rnti << (uint32_t) m_lcid << p->GetSize () <<"LteRlcAm multicastgroup"<< (uint16_t) m_multicastgroup);
...
// get IP address of UE
  ...
  pCopy->RemoveHeader (pdcpHeader);
  uint16_t SetSequenceNumber = pdcpHeader.GetSequenceNumber ();
  NS_LOG_FUNCTION ("SetSequenceNumber" << SetSequenceNumber);
//YEN
...
NS_LOG_FUNCTION ("1RLCAM m_multicastgroup" << m_multicastgroup);
...
NS_LOG_FUNCTION ("2RLCAM m_multicastgroup" << m_multicastgroup);
          m_macSapProvider->TransmitPdu (params);
          return;
        }
      else
      ...
      if ( packet->GetSize () <= bytes )
        {
          ...
          params.harqProcessId = harqId;
          params.multicastgroup = m_multicastgroup;//yen
NS_LOG_FUNCTION ("3RLCAM m_multicastgroup" << m_multicastgroup);
          m_macSapProvider->TransmitPdu (params);
          return;
        }
      else
  ...
  params.harqProcessId = harqId;
  params.multicastgroup = m_multicastgroup;
NS_LOG_FUNCTION ("4RLCAM m_multicastgroup" << m_multicastgroup);
  m_macSapProvider->TransmitPdu (params);
}
