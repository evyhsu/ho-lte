...
/*uint16_t
LteEnbNetDevice::GetMulticastGroup (void)
{
...
/*-------yen write-----------*/
  ...
  Ipv4Address remoteAddr =  ipv4Header.GetSource ();
  uint16_t Identification =  ipv4Header.GetIdentification ();
  uint16_t Protocol =  ipv4Header.GetProtocol ();
  NS_LOG_LOGIC ("Destination: " << ueAddr <<" Identification: " <<Identification << " Protocol:" << Protocol << " SourceAddr :" << remoteAddr);
  SetUeAddress (ueAddr);
...
//yen
Ipv4Address 
LteEnbNetDevice::SetUeAddress (Ipv4Address ueAddr)
{
...
Ipv4Address 
LteEnbNetDevice::SetRemoteAddress (Ipv4Address remoteAddr)
{
