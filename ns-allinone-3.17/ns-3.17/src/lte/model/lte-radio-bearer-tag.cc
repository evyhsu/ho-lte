...
//yen
LteRadioBearerTag::LteRadioBearerTag (uint16_t rnti, uint8_t lcid, uint8_t layer, uint16_t multicastgroup)
: m_rnti (rnti),
  m_lcid (lcid),
  m_layer (layer),
...
//yen
void
LteRadioBearerTag::SetMulticastGroup (uint16_t multicastgroup)
{
  m_multicastgroup = multicastgroup;
}
...
LteRadioBearerTag::Deserialize (TagBuffer i)
{
  ...
  m_multicastgroup = (uint16_t) i.ReadU8 ();
}
...
//yen
uint16_t
LteRadioBearerTag::GetMulticastGroup () const
{
...
{
  os << "rnti=" << m_rnti << ", lcid=" << (uint16_t) m_lcid << ", layer=" << (uint16_t)m_layer << ", multicastgroup=" << (uint16_t)m_multicastgroup;
}
