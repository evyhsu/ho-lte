...
EpsBearerTag::EpsBearerTag (uint16_t rnti, uint8_t bid, uint16_t multicastgroup)
  : m_rnti (rnti),
    m_bid (bid),
...
//yen
void
EpsBearerTag::SetMulticastGroup (uint16_t multicastgroup)
{
  m_multicastgroup = multicastgroup;
}
...
EpsBearerTag::Deserialize (TagBuffer i)
{
  ...
  m_multicastgroup = (uint16_t) i.ReadU16 ();
}
...
//yen
uint16_t
EpsBearerTag::GetMulticastGroup () const
{
