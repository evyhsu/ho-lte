...
//yen
const char* g_LteEnbRrcStateName[LteEnbRrc::NUM_STATES_1] = 
  {
    "INITIAL_RANDOM_ACCESS",
    "CONNECTION_SETUP",
    "CONNECTION_REJECTED",
...
/*yen add */
UeManager::UeManager (Ptr<LteEnbRrc> rrc, uint16_t rnti, State s, uint16_t multicastgroup)
  : m_lastAllocatedDrbid (0),
    m_rnti (rnti),
    m_imsi (0),
...
UeManager::DoInitialize ()
{
  ...
  // setup the eNB side of SRB0
  {
    ...
    rlc->SetRnti (m_rnti);
    rlc->SetLcId (lcid);
    rlc->SetMulticastGroup (m_multicastgroup);//yen
  ...
  // setup the eNB side of SRB1; the UE side will be set up upon RRC connection establishment
  {
    ...
    pdcp->SetRnti (m_rnti);
    pdcp->SetLcId (lcid);
    pdcp->SetMulticastGroup (m_multicastgroup);//yen
    pdcp->SetLtePdcpSapUser (m_drbPdcpSapUser);
    pdcp->SetLteRlcSapProvider (rlc->GetLteRlcSapProvider ());
    rlc->SetLteRlcSapUser (pdcp->GetLteRlcSapUser ());
//yen
NS_LOG_FUNCTION ("m_multicastgroup" << m_multicastgroup << "m_rnti" << m_rnti);
...
{
  m_imsi = imsi;
  NS_LOG_FUNCTION ("yen SetImsi : " << imsi);
}

void 
UeManager::SetMulticastGroup (uint16_t multicastgroup)
{
  m_multicastgroup = multicastgroup;
  NS_LOG_FUNCTION ("yen multicastgroup : " << multicastgroup);
}
...
//YEN
NS_LOG_FUNCTION ("YEN SetupDataRadioBearer drbid" << drbid << " lcid" << lcid << " bid" << bid );  
...
//yen
  NS_LOG_FUNCTION ("yen_gtpTeid:" << drbInfo->m_gtpTeid);
}
...
{
  NS_LOG_FUNCTION (this << p << (uint16_t) bid << "m_rnti" << m_rnti <<"m_multicastgroup" << m_multicastgroup );

//m_rrc->SetUEaddress (p);//yen
   switch (m_state)
    {
    case INITIAL_RANDOM_ACCESS:
    case CONNECTION_SETUP:
...
//yen
        params.multicastgroup = m_multicastgroup;
        NS_LOG_FUNCTION ("m_multicastgroup" << m_multicastgroup);//yen
        uint8_t drbid = Bid2Drbid (bid);
        NS_LOG_FUNCTION ("drbid" << drbid);//yen
...
/*--------------yen add---------------*/
void
UeManager::SendData (uint8_t bid, Ptr<Packet> p, uint16_t m_multicastgroup)
{
  NS_LOG_FUNCTION (this << p << (uint16_t) bid << "m_rnti" << m_rnti <<"m_multicastgroup" << m_multicastgroup );
...
//yen
        params.multicastgroup = m_multicastgroup;
  NS_LOG_FUNCTION ("multicastgroup" << m_multicastgroup);
        uint8_t drbid = Bid2Drbid (bid);
        LtePdcpSapProvider* pdcpSapProvider = GetDataRadioBearerInfo (drbid)->m_pdcp->GetLtePdcpSapProvider ();
        pdcpSapProvider->TransmitPdcpSdu (params);
      }
...
void
UeManager::RecvRrcIGMPRequest (LteRrcSap::RrcIGMPRequest msg)
{
  NS_LOG_FUNCTION ("received from LteRrcSap RrcIGMPRequest");
}
...
//yen
  NS_LOG_FUNCTION ("params.rnti" << m_rnti <<"params.cellId" << m_rrc->m_cellId <<"mmeUeS1Id" <<m_imsi);
...
//yen
void
UeManager::Stoppackettransmittion ()
{
  NS_LOG_FUNCTION (this);

  Ptr<Packet> packet = Create<Packet>(); 

  Ipv4Address destination = m_rrc->GetRemoteAddress ();
  Ipv4Address source = m_rrc->GetUeAddress ();
  ...
  ipHeader.SetProtocol(100); // IGMP 
  packet->AddHeader(igmpheader); 
  packet->AddHeader(ipHeader); 

  uint16_t rnti = GetRnti ();
  uint8_t bid = m_rrc->GetBid ();
      // data radio bearer
      EpsBearerTag tag;
      tag.SetRnti (rnti);
      tag.SetBid (bid);
      packet->AddPacketTag (tag);
      m_rrc->m_forwardUpCallback (packet);
}
...
{
  NS_LOG_FUNCTION ("m_imsi" << m_imsi);
  return m_imsi;

}
//yen
uint16_t
UeManager::GetMulticastGroup (void)
{
  NS_LOG_FUNCTION ("m_multicastgroup" << m_multicastgroup);
...
//yen
        NS_LOG_INFO("YEN drbToAddModList :" << it->first << " SECOND: " << it->second);
//yen
  /*std::list<LteRrcSap::DrbToAddMod >::iterator it;
  for (rrcd.drbToAddModList.begin (); it != rrcd.drbToAddModList.end (); it++)
                {
                //std::map <uint8_t> Indicator;
              char Indicator[5] = rrcd.drbToAddModList.front ();
                //rrcd.drbToAddModList.front ();
             //NS_LOG_INFO("drbToAddModList:" << Indicator[it]) << "");
                }*/
    }
...
LteEnbRrc::GetUeManager (uint16_t rnti)
{
  ...
  NS_LOG_FUNCTION ("yen GetUeManager it->second"<< it->second);  
  return it->second;
...
{
  m_cellId = cellId;
  NS_LOG_FUNCTION ("yen cellId" << cellId);
}
...
LteEnbRrc::SendData (Ptr<Packet> packet)
{
  ...
  NS_LOG_FUNCTION ("tag.GetMulticastGroup" << tag.GetMulticastGroup () << "tag.GetRnti ()" << tag.GetRnti ());
...
//uint16_t multicastgroup = 
  ueManager->SetMulticastGroup ((uint16_t) tag.GetMulticastGroup ());
 //ueManager->SendData (tag.GetBid (), packet);
  ueManager->SendData (tag.GetBid (), packet, tag.GetMulticastGroup ());
  SetBid (tag.GetBid ());//yen
  return true;
}
...
void 
LteEnbRrc::SetBid (uint8_t bid)
{
  NS_LOG_FUNCTION (this << bid);
  m_bid = bid;
}

uint8_t
LteEnbRrc::GetBid (void)
{
  return m_bid;
}
...
//yen
/*void 
LteEnbRrc::SetUEaddress (Ptr<Packet> packet)
{
  ...
  Ipv4Address m_ueAddr = ueAddr;
  NS_LOG_LOGIC ("yen get UE address " << ueAddr);
}

Ipv4Address
LteEnbRrc::GetUEaddress () 
{
  return m_ueAddr;
}*/
/*--------------------yen add----------------------*/
void
LteEnbRrc::SetUeAddress (Ipv4Address ueAddr)
{
...
Ipv4Address
LteEnbRrc::GetUeAddress (void)
{
  NS_LOG_FUNCTION (this << m_ueAddr);
  return m_ueAddr;
}
void
LteEnbRrc::SetRemoteAddress (Ipv4Address remoteAddr)
{
...
Ipv4Address
LteEnbRrc::GetRemoteAddress (void)
{
...
//yen add igmp
void
LteEnbRrc::SendIGMPReport (uint16_t rnti, uint16_t cellId, Ptr<Packet> packet)
{
  NS_LOG_FUNCTION (this << rnti << cellId);
  NS_LOG_LOGIC ("Request to send IGMP REQUEST");
  NS_ASSERT (m_configured);
  Ptr<UeManager> ueManager = GetUeManager (rnti);
...
//yen
void
LteEnbRrc::DoRecvRrcIGMPRequest (uint16_t rnti, LteRrcSap::RrcIGMPRequest msg)
{
  NS_LOG_FUNCTION (this << rnti);
  
    GetUeManager (rnti)->RecvRrcIGMPRequest (msg);
}
...
//yen
  EpcEnbS1SapProvider::IGMPReportParameters params;
  params.rnti = rnti;
  params.cellId = req.targetCellId;
  params.mmeUeS1Id =req.mmeUeS1apId;
  ...
  //ueManager->SwitchToState ();
  //m_s1SapProvider->IGMPReport (params);
}
...
//yen
  //Ptr<Packet> packet;
  //SendIGMPReport (rnti, params.targetCellId, packet);
}
