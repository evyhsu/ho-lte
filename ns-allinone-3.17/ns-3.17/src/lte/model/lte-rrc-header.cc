...
IGMPHeader::Serialize (Buffer::Iterator start) const
{
  ...
  i.WriteU32 (m_groupaddress);
  uint8_t secondByte = m_resv << 4 | m_s << 7 | m_qrv << 5;
  i.WriteU8 (secondByte);
  i.WriteU8 (m_qqic);
  i.WriteU16 (m_numberofsource);
...
IGMPHeader::Deserialize (Buffer::Iterator start)
{
  ...
  m_type = i.ReadU8 ();
  m_maxreptime = i.ReadU8 ();
  m_checksum = i.ReadU8 ();
  m_groupaddress = i.ReadU32 ();
  uint8_t secondByte = i.ReadU8 ();
  m_resv = secondByte >> 4 & 0xF;
  m_s = secondByte >> 7 & 0x1;
  m_qrv = secondByte >> 5 & 0x7;
  m_qqic = i.ReadU8 ();
  m_numberofsource = i.ReadU16 ();
...
IGMPHeader::Print (std::ostream &os) const
{
  ...
  os << " maxreptime=" << (uint32_t) m_maxreptime;
  os << " checksum=" << (uint32_t) m_checksum;
  os << " groupaddress=" << (uint32_t) m_groupaddress;
}

uint8_t
IGMPHeader::GetType () const
{
  return m_type;
}
...
uint8_t
IGMPHeader::GetMaxRepTime () const
{
  return m_maxreptime;
}
...
uint16_t
IGMPHeader::GetChecksum () const
{
  return m_checksum;
}

void
IGMPHeader::SetChecksum (uint16_t checksum)
{
  m_checksum = checksum;
}

uint32_t
IGMPHeader::GetGroupAddress () const
{
  return m_groupaddress;
}

void
IGMPHeader::SetGroupAddress (uint32_t groupaddress)
{
  m_groupaddress = groupaddress;
}

uint8_t
IGMPHeader::GetResv () const
{
  return m_resv;
}

void
IGMPHeader::SetResv (uint8_t resv)
{
  m_resv = resv;
}

uint8_t
IGMPHeader::GetS () const
{
  return m_s;
}

void
IGMPHeader::SetS (uint8_t s)
{
  m_s = s;
}

uint8_t
IGMPHeader::GetQRV () const
{
  return m_qrv;
}

void
IGMPHeader::SetQRV (uint8_t qrv)
{
  m_qrv = qrv;
}
...
uint8_t
IGMPHeader::GetQQIC () const
{
  return m_qqic;
}

void
IGMPHeader::SetQQIC (uint8_t qqic)
{
  m_qqic = qqic;
}

uint16_t
IGMPHeader::GetNumberofSource () const
{
  return m_numberofsource;
}

void
IGMPHeader::SetNumberofSource (uint16_t numberofsource)
{
  m_numberofsource = numberofsource;
}
  ...
  for (; it != drbToAddModList.end (); it++)
    {
      ...
      SerializeInteger (it->drbIdentity,1,32);//decide how many bits it needs #yen
//yen
      NS_LOG_INFO ("yen it->drbIdentity: " << it->drbIdentity << "it->epsBearerIdentity" << it->epsBearerIdentity);
...
//////////////////// yen RrcIGMPRequest class ////////////////////////

// Constructor
RrcIGMPRequestHeader::RrcIGMPRequestHeader () : RrcUlCcchMessage ()
{
  m_mmec = std::bitset<8> (0ul);
  m_mTmsi = std::bitset<32> (0ul);
  m_establishmentCause = MO_SIGNALLING;
...
// Destructor
RrcIGMPRequestHeader::~RrcIGMPRequestHeader ()
{
}

TypeId
RrcIGMPRequestHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::RrcIGMPRequestHeader")
    .SetParent<Header> ()
  ;
  return tid;
}
...
void
RrcIGMPRequestHeader::Print (std::ostream &os) const
{
  os << "MMEC:" << m_mmec << std::endl;
  os << "MTMSI:" << m_mTmsi << std::endl;
  os << "EstablishmentCause:" << m_establishmentCause << std::endl;
...
void
RrcIGMPRequestHeader::PreSerialize () const
{
  m_serializationResult = Buffer ();
...
uint32_t
RrcIGMPRequestHeader::Deserialize (Buffer::Iterator bIterator)
{
  std::bitset<1> dummy;
  std::bitset<0> optionalOrDefaultMask;
  int selectedOption;
...
void
RrcIGMPRequestHeader::SetMessage (LteRrcSap::RrcIGMPRequest msg)
{
  m_mTmsi = std::bitset<32> ((uint32_t)msg.ueIdentity);
  m_mmec = std::bitset<8> ((uint32_t)(msg.ueIdentity >> 32));
  m_isDataSerialized = false;
...
LteRrcSap::RrcIGMPRequest
RrcIGMPRequestHeader::GetMessage () const
{
  LteRrcSap::RrcIGMPRequest msg;
  msg.ueIdentity = (((uint64_t) m_mmec.to_ulong ()) << 32) | (m_mTmsi.to_ulong ());
...
std::bitset<8>
RrcIGMPRequestHeader::GetMmec () const
{
  return m_mmec;
}

std::bitset<32>
RrcIGMPRequestHeader::GetMtmsi () const
{
  return m_mTmsi;
}
