...
public:
  ...
  void SetGroupAddress (uint32_t groupaddress);
  void SetResv (uint8_t resv);//4 bits
  void SetS (uint8_t s);//1 bits
  void SetQRV (uint8_t qrv);//3 bits
  void SetQQIC (uint8_t qqic);
  void SetNumberofSource (uint16_t numberofsource);

  uint8_t GetType () const;
  uint8_t GetMaxRepTime () const;
  uint16_t GetChecksum () const;
  uint32_t GetGroupAddress () const;
  uint8_t GetResv () const;
  uint8_t GetS () const;
  uint8_t GetQRV () const;
  uint8_t GetQQIC () const;
  uint16_t GetNumberofSource () const;
private:
  uint8_t m_type;
  uint8_t m_maxreptime;
  uint16_t m_checksum;
  uint32_t m_groupaddress;
  uint8_t m_resv;//4 bits
  uint8_t m_s;//1 bits
  uint8_t m_qrv;//3 bits
  uint8_t m_qqic;
  uint16_t m_numberofsource;
...
//yen add igmp
class RrcIGMPRequestHeader : public RrcUlCcchMessage
{
public:
  RrcIGMPRequestHeader ();
  ~RrcIGMPRequestHeader ();
   ...
   * @param msg The information element to parse
   */
  void SetMessage (LteRrcSap::RrcIGMPRequest msg);

  /**
   * Returns a RrcConnectionRequest IE from the values in the class attributes
   * @return A RrcConnectionRequest, as defined in LteRrcSap
   */
  LteRrcSap::RrcIGMPRequest GetMessage () const;
