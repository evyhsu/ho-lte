...
//yen add iterator yenit
  std::map <uint16_t, DlHarqProcessesBuffer_t>::iterator yenit;
          for (yenit = m_miDlHarqProcessesPackets.begin (); yenit != m_miDlHarqProcessesPackets.end (); ++yenit)
          
                        {
                NS_LOG_INFO ("yen m_miDlHarqProcessesPackets first: " << yenit->first );//<< " second: " << yenit->second);  
                        }
}
...
params.multicastgroup = multicastgroup;
  NS_LOG_FUNCTION (this << "yen params.multicastgroup" << params.multicastgroup << "params.rnti" << params.rnti);

  //LteRadioBearerTag tag (params.rnti, params.lcid, params.layer);
  LteRadioBearerTag tag (params.rnti, params.lcid, params.layer, params.multicastgroup);
...
//yen
  NS_LOG_INFO ("yen_Rnti: " <<(uint16_t)tag.GetRnti () << " lcid: " << (uint16_t)tag.GetLcid () << " yen_DoTransmitPdu");
...
NS_LOG_FUNCTION ("params.pdu" << params.pdu);//YEN
...
LteEnbMac::DoDlInfoListElementHarqFeeback (DlInfoListElement_s params)
{
  ...
  NS_LOG_FUNCTION ("YEN m_miDlHarqProcessesPackets first" << it->first);
  NS_ASSERT (it!=m_miDlHarqProcessesPackets.end ());
