...
/*---------------yen modify-------------*/
void 
EpcMme::AddUe (uint64_t imsi, uint16_t multicastgroup)
{
  NS_LOG_FUNCTION (this << imsi << multicastgroup);
  Ptr<UeInfo> ueInfo = Create<UeInfo> ();
  ueInfo->imsi = imsi;
  ueInfo->mmeUeS1Id = imsi;
  ueInfo->multicastgroup = multicastgroup;
  m_ueInfoMap[imsi] = ueInfo;
  ueInfo->bearerCounter = 0;
  NS_LOG_FUNCTION ("YEN m_ueInfoMap[imsi]" << m_ueInfoMap[imsi] << "ueInfo->mmeUeS1Id" <<ueInfo->mmeUeS1Id << "ueInfo->multicastgroup" <<ueInfo->multicastgroup);
}
...
//yen
  uint16_t multigroup = it->second->multicastgroup;
  NS_LOG_FUNCTION ("YEN it->second->multicastgroup" << it->second->multicastgroup);
  ...
  jt->second->s1apSapEnb->InitialContextSetupRequest (mmeUeS1Id, enbUeS1Id, erabToBeSetupList, multigroup);
}
