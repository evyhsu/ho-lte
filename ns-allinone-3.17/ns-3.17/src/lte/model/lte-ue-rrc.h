...
#include <ns3/epc-tft-classifier.h>//yen
#include "lte-ue-net-device.h"//yen
#include "epc-sgw-pgw-application.h"//yen
#include <map>
...
/*---------yen add-------------*/
   ...
   */
  State GetState ();
  IgmpState GetIgmpState ();//yen
  ReceiveState GetReceiveState ();//yen
...
private:
  ...
  void DoSendigmp (uint8_t bid);//yen
  void DoNotifyRandomAccessFailed ();
  void SetBid (uint8_t bid);//yen
  uint8_t GetBid () const;//yen
  ...
  void SwitchToIgmpState (IgmpState s);//yen
  void SwitchToReceiveState (ReceiveState s);//yen
  std::map<uint8_t, uint8_t> m_bid2DrbidMap;

  //yen
  EpcTftClassifier m_tftClassifier;
  uint32_t m_identification;//yen
  uint16_t m_bid;//yen
//yen
  /*Ptr<LteUeNetDevice> m_uedev;
  Ptr<EpcSgwPgwApplication> m_sgwpgw;
  Ptr<LteEnbRrc> m_enbrrc;*/
  //Ptr<UeInfo> m_ueinfo;//can't work
  ...
  IgmpState m_igmpstate;//yen

  ReceiveState m_receivestate;//yen

  uint64_t m_imsi;
  uint16_t m_rnti;
  uint16_t m_cellId;
  uint16_t m_multicastgroup; //yen
