...
{
  NS_LOG_FUNCTION ("EpcSgwPgwApplication::SetEnbAddr" << enbAddr );
  m_enbAddr = enbAddr;
}
...
void
EpcSgwPgwApplication::UeInfo::SetUemulti (uint16_t multicastgroup)
{
  //uint16_t multicastgroup = 20;
  m_multicastgroup = multicastgroup;
}

uint16_t
EpcSgwPgwApplication::UeInfo::GetUemulticast ()
{
...
bool
EpcSgwPgwApplication::RecvFromTunDevice (Ptr<Packet> packet, const Address& source, const Address& dest, uint16_t protocolNumber)//internet->gw
{
  ...
  uint16_t Protocol =  ipv4Header.GetProtocol ();//YEN 
  NS_LOG_LOGIC ("Protocol " << Protocol << "SIZE " << packet->GetSize ());
  ...
  if (it == m_ueInfoByAddrMap.end ())
  ...
  else
    {
      ...
      NS_LOG_LOGIC ("teid " << teid << " enbAddr" << enbAddr);
//yen
      //uint32_t teid = 65535;   
      if (teid == 0)
        {
...
void 
EpcSgwPgwApplication::SendToTunDevice (Ptr<Packet> packet, uint32_t teid)//pgw->internet
{
  NS_LOG_FUNCTION (this << packet << teid);
  NS_LOG_LOGIC (" packet size: " << packet->GetSize () << " bytes");
  NS_LOG_FUNCTION ("yen m_tunDevice->GetAddress ()" << m_tunDevice->GetAddress ());//yen
//yen
  // get IP address of UE
  Ptr<Packet> pCopy = packet->Copy ();
  Ipv4Header ipv4Header;
  pCopy->RemoveHeader (ipv4Header);
  Ipv4Address  remotehostAddr =  ipv4Header.GetDestination ();
  ...
  NS_LOG_LOGIC ("Destination: " << remotehostAddr <<" Identification: " <<Identification << " Protocol:" << Protocol << " SourceAddr :" << ueAddr);

  m_tunDevice->Receive (packet, 0x0800, m_tunDevice->GetAddress (), m_tunDevice->GetAddress (), NetDevice::PACKET_HOST);
  Sendicmp (remotehostAddr, ueAddr);
}
void 
EpcSgwPgwApplication::Sendicmp (Ipv4Address remotehostAddr, Ipv4Address ueAddr)//pgw->internet
{
  NS_LOG_FUNCTION ("send icmp to internet");

  NS_LOG_FUNCTION ("ueAddr: " << ueAddr << "remotehostAddr" << remotehostAddr);

  Ptr<Packet> p = Create<Packet>(); 
  Icmpv4Echo echo; 
  echo.SetSequenceNumber(36); 
  ...
  Ipv4Header ipHeader; 
  ipHeader.SetDestination(remotehostAddr); 
  ipHeader.SetSource(ueAddr); 
  ...
  m_identification++;
  m_tunDevice->Receive (p, 0x0800, m_tunDevice->GetAddress (), m_tunDevice->GetAddress (), NetDevice::PACKET_HOST);
}
//yen
/*void 
EpcSgwPgwApplication::SendToTunDevice (Ptr<Packet> packet, uint32_t teid)
{
  NS_LOG_FUNCTION (this << packet << teid);
  NS_LOG_LOGIC (" packet size: " << packet->GetSize () << " bytes");
  m_tunDevice->Receive (packet, 0x0800, m_tunDevice->GetAddress (), m_tunDevice->GetAddress (), NetDevice::PACKET_MULTICAST );
}*/
...
{
  NS_LOG_FUNCTION ("m_s11SapSgw"<< m_s11SapSgw);
  return m_s11SapSgw;
}
...
EpcSgwPgwApplication::AddEnb (uint16_t cellId, Ipv4Address enbAddr, Ipv4Address sgwAddr)
{
  ...
  //NS_LOG_FUNCTION ("yen enbInfo" << enbInfo);
}
...
//yen
void 
EpcSgwPgwApplication::AddUe (uint64_t imsi, uint16_t multicastgroup)
{
  NS_LOG_FUNCTION (this << imsi << multicastgroup);
  Ptr<UeInfo> ueInfo = Create<UeInfo> ();
  m_ueInfoByImsiMap[imsi] = ueInfo;
  NS_LOG_FUNCTION ("yen ueInfo" << ueInfo);

  Ptr<UeInfo> uemulticastInfo = Create<UeInfo> ();
  m_ueInfoByMulticastGroupMap[multicastgroup] = uemulticastInfo;
  NS_LOG_FUNCTION ("yen uemulticastInfo" << uemulticastInfo);
}
...
//yen
  NS_LOG_FUNCTION ("yen m_ueInfoByImsiMap" << ueit->second);

  /*std::map<uint16_t, Ptr<UeInfo> >::iterator mulit = m_ueInfoByMulticastGroupMap.find (multicastgroup) ;
  NS_ASSERT_MSG (mulit != m_ueInfoByMulticastGroupMap.end (), "unknown multicastgroup " << multicastgroup);
  m_ueInfoByMulticastMap[multicastgroup] = mulit->second;
  mulit->second->SetUemulti (multicastgroup); 
  NS_LOG_FUNCTION ("yen m_ueInfoByMulticastGroupMap" << mulit->second);*/
}
//yen
Ipv4Address 
EpcSgwPgwApplication::GetUeAddress ()
{
  Ptr<UeInfo> UeInfo;
  UeInfo->GetUeAddr ();
  return m_ueAddress;
...
//yen
  std::map<uint16_t, Ptr<UeInfo> >::iterator mulit;
  //NS_LOG_FUNCTION ("yen UemulticastInfo " << mulit->first);
...
//yen
      //uint32_t teid = 65535;  
      NS_LOG_FUNCTION ("yen DoCreateSessionRequest teid :" << teid << "ueit->second" << ueit->second);

      ueit->second->AddBearer (bit->tft, bit->epsBearerId, teid);
      //mulit->second->AddBearer (bit->tft, bit->epsBearerId, teid);
