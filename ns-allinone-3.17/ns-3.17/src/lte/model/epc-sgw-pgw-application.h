...
public:
  ...
  void Sendicmp (Ipv4Address ueAddr, Ipv4Address sgwAddr);
...
//yen
  void AddUe (uint64_t imsi, uint16_t multicastgroup);
  /** 
...
//yen
  ...
  //void SetUeAddress (uint64_t imsi, Ipv4Address ueAddr, uint16_t multicastgroup);
  //void SetUemulticast (uint64_t imsi, uint16_t multicastgroup);
...
//yen
    void SetUemulti (uint16_t multicastgroup);
    uint16_t GetUemulticast ();
  private:
...
//yen
  std::map<uint16_t, Ptr<UeInfo> > m_ueInfoByMulticastGroupMap;
  std::map<uint16_t, Ptr<UeInfo> > m_ueInfoByMulticastMap;
  /**
...
/*--------------------------------yen create ----------------------*/
 /* struct UemulticastInfo
  {
    void AddBearer (Ptr<EpcTft> tft, uint8_t epsBearerId, uint32_t teid);
  };
  std::map<uint16_t, UemulticastInfo > m_ueInfoByMulticastGroupMap;*/
