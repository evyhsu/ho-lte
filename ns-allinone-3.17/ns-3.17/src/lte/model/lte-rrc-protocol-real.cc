...
//yen
void 
LteUeRrcProtocolReal::DoSendRrcIGMPRequest (LteRrcSap::RrcIGMPRequest msg)
{
  m_rnti = m_rrc->GetRnti ();
  SetEnbRrcSapProvider ();
  ...
  rrcIGMPRequestHeader.SetMessage (msg);

  packet->AddHeader (rrcIGMPRequestHeader);
...
LteUeRrcProtocolReal::DoSendRrcConnectionReconfigurationCompleted (LteRrcSap::RrcConnectionReconfigurationCompleted msg)
{
  ...
  // eNB we are currently attached to
  NS_LOG_FUNCTION ("yen LteUeRrcProtocolReal DoSendRrcConnectionReconfigurationCompleted");
  m_rnti = m_rrc->GetRnti ();
  SetEnbRrcSapProvider ();
...
{
NS_LOG_FUNCTION ("YEN DoSendRrcConnectionReconfiguration");
  Ptr<Packet> packet = Create<Packet> ();
