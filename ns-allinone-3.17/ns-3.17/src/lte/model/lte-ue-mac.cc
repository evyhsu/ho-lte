...
//yen
  NS_LOG_FUNCTION ("params.rnti" << params.rnti << "params.multicastgroup" << params.multicastgroup/* << "  params.harqProcessId" << (uint8_t) params.harqProcessId*/);
}
...
{
  NS_LOG_FUNCTION (this << params.multicastgroup/*yen*/);
  NS_LOG_FUNCTION ("params.multicastgroup" << params.multicastgroup << "params.rnti" << params.rnti << "rnti" <<m_rnti );
...
//yen
   //NS_LOG_FUNCTION ("YEN m_lcInfoMap lcInfo" << lcId );
 std::map <uint8_t, LcInfo>::iterator it;
 for (it = m_lcInfoMap.begin (); it != m_lcInfoMap.end (); it++)
        {       
         //NS_LOG_FUNCTION ("YEN m_lcInfoMap it->first" << it->first);
...
{
//yen
NS_LOG_FUNCTION ("p "<< p << "packet size" <<p->GetSize () );
  LteMacSapProvider::TransmitPduParameters params;
  uint16_t multicastgroup = 40;
  params.multicastgroup = multicastgroup;

  NS_LOG_FUNCTION ("yen DoReceivePhyPdu packet" << p << " rnti" << m_rnti << "params.multicastgroup" << params.multicastgroup );
  NS_LOG_INFO ( "rnti: " << m_rnti << " yen_DoReceivePhyPdu");//YEN
  LteRadioBearerTag tag;
  p->RemovePacketTag (tag);
  NS_LOG_FUNCTION ("tag.multicastgroup" << tag.GetMulticastGroup ());//YEN
  if (tag.GetRnti () == m_rnti)
  //if (tag.GetMulticastGroup () == params.multicastgroup)
    {
...
//yen
      //NS_LOG_FUNCTION ("yen DoReceivePhyPdu lcid " << lcid);
      //std::map <uint8_t, LcInfo>::const_iterator it;
      //NS_LOG_FUNCTION ("yes i am in " );//YEN
...
LteUeMac::DoSubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
  NS_LOG_FUNCTION (this);
//yen
  NS_LOG_FUNCTION ("yen lteuemac frameno"<< frameNo << " subframeNo" << subframeNo << "M_RNTI" << m_rnti);
  m_frameNo = frameNo;
