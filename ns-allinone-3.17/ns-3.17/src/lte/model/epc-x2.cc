...
EpcX2::RecvFromX2uSocket (Ptr<Socket> socket)
{
  ...
  Ptr<Packet> packet = socket->Recv ();
  NS_LOG_LOGIC ("packetLen = " << packet->GetSize () << ", packet = " << packet);
...
//yen
  NS_LOG_FUNCTION ("YEN DoSendUeData forward packet :" << packet << "packet length" << packet->GetSize () << "params.ueData" << params.ueData);
