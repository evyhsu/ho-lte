...
/*--------------yen modify----------------*/
  LteUeNetDevice::LteUeNetDevice (Ptr<Node> node, Ptr<LteUePhy> phy, Ptr<LteUeMac> mac, Ptr<LteUeRrc> rrc, Ptr<EpcUeNas> nas, uint64_t imsi, uint16_t multicastgroup)
{
  NS_LOG_FUNCTION (this);
  m_phy = phy;
  m_mac = mac;
...
//yen
  m_nas->SetMulticastGroup (m_multicastgroup);
  m_rrc->SetMulticastGroup (m_multicastgroup);
...
/*-------yen write-----------*/
  ...
  m_rrc->SetUeAddress (ueAddr);
  m_rrc->SetRemoteAddress (remoteAddr);
/*-------yen end-----------*/
...
//yen
Ipv4Address 
LteUeNetDevice::SetUeAddress (Ipv4Address ueAddr)
{
  Ipv4Address m_ueAddress = ueAddr;
  NS_LOG_FUNCTION ("SetUeAddress :" << m_ueAddress);
  return m_ueAddress;
}

Ipv4Address 
LteUeNetDevice::SetRemoteAddress (Ipv4Address remoteAddr)
{
  Ipv4Address m_remoteAddr = remoteAddr;
  NS_LOG_FUNCTION ("remoteAddr :" << m_remoteAddr);
  return m_remoteAddr;
}


/*--------------------------yen set ue multicastgroup--------------------------*/

uint16_t
LteUeNetDevice::GetMulticastGroup (void)
{
