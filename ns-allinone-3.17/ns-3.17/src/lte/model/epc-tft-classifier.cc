...
EpcTftClassifier::Add (Ptr<EpcTft> tft, uint32_t id)
{
  ...
  m_tftMap[id] = tft; 
  NS_LOG_FUNCTION ("yen m_tftmap:" << m_tftMap[id] <<"yen id:" <<id << "tft" << tft); 
//yen
  std::map <uint32_t, Ptr<EpcTft> > ::iterator it;
  for (it = m_tftMap.begin (); it != m_tftMap.end (); it++)
{
  NS_LOG_FUNCTION ("yen m_tftmap it->first :" << it->first <<" it->second: " << it->second);
}
...
EpcTftClassifier::Classify (Ptr<Packet> p, EpcTft::Direction direction)
{
  ...
  if (protocol == UdpL4Protocol::PROT_NUMBER)
      ...
      else
	{
      ...
      NS_LOG_FUNCTION ("remotePort: " << remotePort << " localPort:" << localPort );
	}
    }
...
//yen add for igmp
	...
	{
      NS_LOG_FUNCTION ("EpcTftClassifier receive DL protocol=2");//yen
	  remotePort = udpHeader.GetSourcePort ();
	  localPort = udpHeader.GetDestinationPort ();
	}
    }
  ...
  for (it = m_tftMap.rbegin (); it != m_tftMap.rend (); ++it)
    {
	  ...
	  //return  65532; // the id of the matching TFT
      NS_LOG_INFO ("yen tft id :" << it->first);
        }
    }
...
return  65532;*/
}//end for EpcTftClassifier::Classify
