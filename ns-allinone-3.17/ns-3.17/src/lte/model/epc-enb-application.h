...
///yen
    EpsFlowId_t (const uint16_t a, const uint8_t b, const uint16_t c);
...
private:
  ...
  void DoIGMPReport (EpcEnbS1SapProvider::IGMPReportParameters params);
  ...
  void DoInitialContextSetupRequest (uint64_t mmeUeS1Id, uint16_t enbUeS1Id, std::list<EpcS1apSapEnb::ErabToBeSetupItem> erabToBeSetupList, uint16_t multigroup);
...
//yen
  void SendToLteSocket (Ptr<Packet> packet, uint16_t rnti, uint8_t bid, uint16_t multicastgroup);
