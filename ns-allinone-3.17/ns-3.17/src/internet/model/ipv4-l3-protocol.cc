...
//yen add for igmp switch
const char* g_ueRrcStateName2[Ipv4L3Protocol::NUM_STATES_igmp] = 
  {
    "CONNECTED_ON",
    "CONNECTED_OFF"
  };

std::string ToString (Ipv4L3Protocol::IgmpState s)
{
  return std::string (g_ueRrcStateName2[s]);
}
...
/*----------------yen add to judge if sgw really send pkt back----------------------*/
if (m_node->GetId () == 1)
  {
  uint16_t Identification =  ipHeader.GetIdentification ();
  uint16_t Protocol =  ipHeader.GetProtocol ();
  NS_LOG_FUNCTION ("Identification: " << Identification << " ,Protocol: " << Protocol);
  }
if (m_node->GetId () == 5)
  {
  uint16_t Identification =  ipHeader.GetIdentification ();
  ...
  NS_LOG_FUNCTION ("Identification: " << Identification << " ,Protocol: " << Protocol << ",source:" << source << ",destination:" << destination);
  }

if (m_node->GetId () == 0)
  {
  uint16_t Identification =  ipHeader.GetIdentification ();
  NS_LOG_FUNCTION ("node0, Identification: " << Identification);
  }
  if (m_node->GetId () == 2)
      {
    uint16_t Identification =  ipHeader.GetIdentification ();
    NS_LOG_FUNCTION ("from the front node2->router, Identification: " << Identification);

      }
  uint16_t Protocol =  ipHeader.GetProtocol ();
  NS_LOG_FUNCTION (" ALL PACKET'S  Protocol: " << Protocol);
...
/*-----------------yen add for igmp-------------*/
  if (m_node->GetId () == 2)
      {
  uint16_t Identification =  ipHeader.GetIdentification ();
  NS_LOG_FUNCTION ("node2->router, Identification: " << Identification);
  uint16_t Protocol =  ipHeader.GetProtocol ();
  if (Protocol == 2)
                {
           NS_LOG_FUNCTION ("this function is received by router and is IGMP");
           bool isreceived = 1;
           SetIgmpreceived (isreceived);
           SwitchToIgmpState(CONNECTED_ON);

                }
      }
/*-----------------yen add for stop packet-------------*/
  if (m_node->GetId () == 2)
      {
  uint16_t Protocol =  ipHeader.GetProtocol ();

  //Ipv4Address GroupAddress = igmpheader.GetGroupAddress ();
  if (Protocol == 100)
                {
           NS_LOG_FUNCTION ("this function is received by router and have to stop transmit packet");

  IgmpHeader igmpheader; 
  uint32_t type = igmpheader.GetType ();
           NS_LOG_FUNCTION ("type " << type );
           if (type == 1)
                        {
             NS_LOG_FUNCTION ("type is 17 " );
                        }
           bool isreceived = 0;
           SetIgmpreceived (isreceived);
                }
...
//TEST FOR ICMP
  ...
  if (m_node->GetId () == 2)
      {
        uint16_t Protocol =  ipHeader.GetProtocol ();
        Icmpv4Echo icmpHeader;
        uint16_t SequenceNumber = icmpHeader.GetSequenceNumber ();
  if (Protocol == 1)
                {
           NS_LOG_FUNCTION ("this function is received by router and is ICMP");
           NS_LOG_FUNCTION ("SequenceNumber" << SequenceNumber);
                }
      }
...
/*-----------yen add for extract ip address------------*/
 void
Ipv4L3Protocol::SetRemoteAddress (Ipv4Address remoteAddr)
{
...
Ipv4Address
Ipv4L3Protocol::GetRemoteAddress (void)
{
  NS_LOG_FUNCTION (this << m_remoteAddr);
  return m_remoteAddr;
}

void
Ipv4L3Protocol::SetUeAddress (Ipv4Address ueAddr)
{
...
Ipv4Address
Ipv4L3Protocol::GetUeAddress (void)
{
  NS_LOG_FUNCTION (this << m_ueAddr);
  return m_ueAddr;
}
  ...
  if (m_routingProtocol != 0)
    {
      ...
      NS_LOG_FUNCTION ("case 5 newroute " << newRoute <<" packet " <<packet );
    }
  else
    {
  ...
  if (newRoute)
    {
      ...
      NS_LOG_FUNCTION ("YEN interface: " << interface);
      m_sendOutgoingTrace (ipHeader, packet, interface);
                             ...
                             Ipv4Header const &ipHeader)
{
  ...
  NS_LOG_LOGIC ("outDev: "<< outDev << " outInterface: "<< outInterface);//yen
  NS_LOG_LOGIC ("Send via NetDevice ifIndex " << outDev->GetIfIndex () << " ,ipv4InterfaceIndex: " << interface << " ,node id: " << m_node->GetId ());


/*if (outDev->GetIfIndex () == 1 && interface == 1 && m_node->GetId () == 0)
{
  SetOutputDeviceyen (outDev);
  Ipv4Address gw = route->GetGateway ();
  Ipv4Address destination =  ipHeader.GetDestination ();
  Ipv4Address source =  ipHeader.GetSource ();
  SetRemoteAddress (source);
  SetUeAddress (destination);
  NS_LOG_LOGIC ("YEN source "<< source << " destination: "<< destination);//yen
  SendIGMPReply (route->GetOutputDevice (), outInterface,gw);
...
//yen add 
if (outDev->GetIfIndex () == 2 && m_node->GetId () == 2)
        {
        NS_LOG_LOGIC ("YES CAN WORK");
      bool isIgmpreceived = GetIgmpreceived ();
        //NS_LOG_LOGIC ("isIgmpreceived" << isIgmpreceived);
      if ( isIgmpreceived == 0)
                {
          NS_LOG_LOGIC ("isIgmpreceived == 0" );
          return;
                }
      if (isIgmpreceived == 1)
{

          NS_LOG_LOGIC ("isIgmpreceived == 1" );
          ...
          switch (m_igmpstate)
            ...
            case CONNECTED_ON:
              ...
              SendIGMPReply (packet, outDev, outInterface, gw);
              SwitchToIgmpState(CONNECTED_OFF);

            case CONNECTED_OFF:
              break;

            default:
              NS_FATAL_ERROR ("method unexpected in state " << ToString (m_igmpstate));
              break;
                    }
...
//yen
void
Ipv4L3Protocol::SendIGMPReply (Ptr<Packet> packet, Ptr<NetDevice> outDev, Ptr<Ipv4Interface> outInterface, Ipv4Address gw)
{
        Ptr<Packet> p = packet;
        Ptr<Packet> pCopy = p->Copy ();
        Ptr<NetDevice> outDevyen = outDev;
        int32_t interface = GetInterfaceForDevice (outDevyen);
        NS_ASSERT (interface >= 0);
        Ptr<Ipv4Interface> outInterfaceyen = outInterface;
        Ipv4Address gwyen = gw;
        NS_LOG_FUNCTION ("outDev: " << outDevyen << " outInterfaceyen" << outInterfaceyen << " gwyen" << gwyen);

        Ipv4Address source = GetRemoteAddress ();
        Ipv4Address destination = GetUeAddress ();

        Ipv4Header ipHeader;
        pCopy->RemoveHeader (ipHeader);//in other to remove old header and add our own


        bool mayFragment = true;
        uint8_t ttl = m_defaultTtl;
        uint8_t tos = m_defaultTos;
        uint8_t protocol =  2;//YEN 
        uint8_t PayloadSize =  28;//YEN 

        IgmpHeader igmpheader; 
        igmpheader.SetType (0x16); 
        igmpheader.EnableChecksum(); 
        ipHeader = BuildHeader (source, destination, protocol, PayloadSize, ttl, tos, mayFragment);
            /*Ipv4Header ipHeader; 
            ipHeader.SetDestination(destination); 
            ipHeader.SetSource(source); 
            ipHeader.SetProtocol(15); // IGMP 
            ipHeader.SetPayloadSize(28); // 20+8 
            ipHeader.SetTtl(16); 
            ipHeader.SetIdentification (m_identification);
            ipHeader.EnableChecksum();*/ 
         pCopy->AddHeader(igmpheader); 
         pCopy->AddHeader(ipHeader); 
         m_identification++;

         Ptr<Ipv4Route> route = Create<Ipv4Route> ();
         route->SetSource (ipHeader.GetSource ());
         route->SetDestination (ipHeader.GetDestination ());
         route->SetGateway (gwyen);
         Ptr<NetDevice> routedevice= GetOutputDeviceyen ();
         NS_LOG_FUNCTION ("routedevice is :" << routedevice);
         route->SetOutputDevice (routedevice);

          if (route == 0)
                    {
              NS_LOG_WARN ("No route to host.  Drop.");
              m_dropTrace (ipHeader, pCopy, DROP_NO_ROUTE, m_node->GetObject<Ipv4> (), 0);
              return;
                    }

              m_txTrace (pCopy, m_node->GetObject<Ipv4> (), interface);
              //outInterface->Send (p, ipHeader.GetDestination ());
              outInterface->Send (pCopy, route->GetGateway ());
 }
...
void
Ipv4L3Protocol::SetOutputDeviceyen (Ptr<NetDevice> outputDevice)
{
...
Ptr<NetDevice>
Ipv4L3Protocol::GetOutputDeviceyen (void) const
{
  NS_LOG_FUNCTION (this << m_outputDevice);
  return m_outputDevice;
}

//yen add
void 
Ipv4L3Protocol::SwitchToIgmpState (IgmpState newState)
{
  NS_LOG_FUNCTION (this << newState);
  IgmpState oldState = m_igmpstate;
  m_igmpstate = newState;
  NS_LOG_INFO (ToString (oldState) << " --> " << ToString (newState));
  switch (newState)
    {
    case CONNECTED_OFF:
      break;
...
void 
Ipv4L3Protocol::SetIgmpreceived (bool igmpreceived)
{
  NS_LOG_FUNCTION ("igmpreceived: " << igmpreceived);
  m_igmpreceived = igmpreceived;
  //m_igmpreceived = 100;
GetIgmpreceived ();
}

bool
Ipv4L3Protocol::GetIgmpreceived (void) const
{
  NS_LOG_FUNCTION (this);
if (m_igmpreceived == 0)
{
  NS_LOG_FUNCTION ("value=0");
}
if (m_igmpreceived == 1)
{
  NS_LOG_FUNCTION ("value=1");
}
  return m_igmpreceived;
}
